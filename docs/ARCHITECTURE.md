...
GitHub Actions checks Rust and Flutter code, runs project-storage and recovery integration tests on macOS and Windows, builds the macOS, Windows, Linux, and Android targets, and runs a macOS bridge smoke test against CLI bootstrap data. The `.orproj` v1 boundary accepts files up to 64 MiB, validates UTF-8, and saves through a same-directory temporary file with platform atomic replacement. Recovery uses a separate bounded sidecar containing the exact saved base and a newer `ProjectDocument` snapshot; inspection is read-only, and load never applies a checkpoint automatically. Applying a candidate revalidates the saved base and atomically saves the recovery snapshot without incrementing its revision. A conflict does not select a winner. This initial snapshot representation may evolve after real scale measurements. Session history remains in-memory; migrations, recovery UI, autosave, and filesystem watching are not implemented. There is no general command/query registry framework; the static catalogs contain only the implemented operations. There is no timeline engine, media engine, renderer, FFmpeg, wgpu, IPC service, or AI system. Prototype behavior is simulated in browser-side code and is not evidence of production architecture.
