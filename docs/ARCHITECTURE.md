...
This is the canonical high-level architecture. Phase 3's executable bootstrap skeleton, Phase 4A–4F project/application foundations, and Phase 4UI-1's Flutter visual foundation are implemented. Phase 4F adds file-backed sessions, local IPC, and semantic CLI operations. Phase 4UI-2 remains: Flutter does not yet own real project create/open/save or host a live project session. Recovery UI and autosave are not implemented. Planned and future components below do not imply implemented code. See [ROADMAP.md](ROADMAP.md) for phases and [TECHNICAL_PLAN.md](TECHNICAL_PLAN.md) for subsystem detail.

## Implemented today

The repository contains a minimal Rust workspace with `or_core`, `or_ipc`, a semantic `or` CLI, and the native Flutter application at `apps/or_app`. `or_core` provides application info, health, and capability discovery; Phase 4A values for project identity, runtime project-instance identity, project revision, exact rational time and rate, and time ranges; a minimal `ProjectDocument` with a strict `.orproj` v1 JSON encoder/decoder and bounded filesystem load/atomic-save APIs; a separate snapshot recovery checkpoint format with bounded read, ancestry inspection, and explicit apply/discard APIs; and a `ProjectSession` with static command/query catalogs, versioned envelopes, `project.rename`, `project.summary`, rename-only atomic transaction groups, normalized `ChangeSet` results, and session-local undo/redo history. `ProjectSession` dispatches the transport-independent `ApplicationRequest` to those existing command, query, and transaction paths. `ProjectFileSession` owns one project path, live session, exact last-saved document, dirty state, recovery policy, and external-change-checked save.

The semantic CLI uses that same dispatch. Headless summary and rename open `ProjectFileSession`; a changed rename uses atomic save. Attached commands use the `or_ipc` client and an explicit endpoint descriptor to reach a live `ProjectFileSession`. `or_ipc` v1 uses bounded framed JSON, per-server authentication, Unix-domain sockets on macOS/Linux, and Windows named pipes with remote clients rejected. Its `Describe`, `Application`, `Save`, and guarded `Shutdown` requests do not accept arbitrary file paths or shell commands. The only CLI server host is the developer/headless `or session serve` command. Flutter currently obtains bootstrap data through generated typed bindings from `flutter_rust_bridge` 2.13; it does not host a live project session. The bridge uses the `crates/or_app_bridge` adapter and `packages/or_app_bridge` Dart package.

Production Flutter implements the production-direction visual shell: Focused Monochrome design tokens, the primary Home/Projects/Templates/Asset Library/Settings surfaces, Advanced / Developer diagnostics through the existing Rust gateway, responsive desktop and compact navigation, and a clearly non-functional Editor Shell Preview. Flutter owns presentation state only. The preview is not canonical project state and is not a timeline engine; there is no media or editing behavior behind it.

GitHub Actions checks Rust and Flutter code, runs project-storage, recovery, and real local IPC integration tests on macOS and Windows, runs the complete Rust workspace (including CLI and Unix IPC tests) on Linux, builds the macOS, Windows, Linux, and Android targets, and runs a macOS bridge smoke test against CLI bootstrap data. The `.orproj` v1 boundary accepts files up to 64 MiB, validates UTF-8, and saves through a same-directory temporary file with platform atomic replacement. Recovery uses a separate bounded sidecar containing the exact saved base and a newer `ProjectDocument` snapshot; inspection is read-only, and load never applies a checkpoint automatically. `ProjectFileSession` checks recovery before opening/saving and compares the exact on-disk document with its saved base before replacement. Applying a recovery candidate revalidates the saved base and atomically saves the snapshot without incrementing its revision. A conflict does not select a winner. This initial snapshot representation may evolve after real scale measurements. Session history remains in-memory; migrations, recovery UI, autosave, filesystem watching, and Flutter project lifecycle/IPC hosting are not implemented. There is no general command/query registry framework; the static catalogs contain only the implemented operations. There is no timeline engine, media engine, renderer, FFmpeg, wgpu, public/network listener, or AI system. Prototype behavior is simulated in browser-side code and is not evidence of production architecture.
...
The CLI is a first-class semantic client of this API. Its parity is for domain operations and meaningful project queries, not presentation-only controls. It supports headless operation and attachment through an explicit descriptor to the developer/headless `or session serve` host. The running Flutter application does not yet host a project session for attached CLI clients. The CLI does not automate the UI by clicking coordinates. Agents are also intended to be clients of structured queries and commands; generated EditPlans must pass the same permission and domain validation as human-initiated work.
...
Today only the developer/headless `or session serve` command hosts a session. The Flutter application does not yet host this session or connect its project lifecycle to IPC; that is the remaining Phase 4UI-2 checkpoint. Android uses platform storage integration, including the Storage Access Framework where appropriate, through a future platform storage abstraction.
