...
This is the canonical high-level architecture. Phase 3's executable bootstrap skeleton, Phase 4A–4F project/application foundations, Phase 4UI-1's Flutter visual foundation, and Phase 4UI-2's desktop project lifecycle are implemented. The Flutter application creates and opens real projects, owns one Rust live host shared with local IPC, and presents explicit recovery and dirty-state workflows. Autosave, Android Storage Access Framework integration, and all timeline/media behavior remain future work. Planned and future components below do not imply implemented code. See [ROADMAP.md](ROADMAP.md) for phases and [TECHNICAL_PLAN.md](TECHNICAL_PLAN.md) for subsystem detail.
