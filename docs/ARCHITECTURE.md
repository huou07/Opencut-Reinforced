...
GitHub Actions checks Rust and Flutter code, runs project-storage integration tests on macOS and Windows, builds the macOS, Windows, Linux, and Android targets, and runs a macOS bridge smoke test against CLI bootstrap data. The filesystem boundary accepts files up to 64 MiB, validates UTF-8, reuses the `.orproj` v1 codec, and saves through a same-directory temporary file with platform atomic replacement. Session history remains in-memory; migrations, crash recovery, autosave, and filesystem watching are not implemented. There is no general command/query registry framework; the static catalogs contain only the implemented operations. There is no timeline engine, media engine, renderer, FFmpeg, wgpu, IPC service, or AI system. Prototype behavior is simulated in browser-side code and is not evidence of production architecture.
