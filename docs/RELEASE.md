...
The current preview includes the production-direction native Flutter shell using OR Focused Monochrome, visual foundations for Home, Projects, Templates, Asset Library, and Settings, responsive desktop and mobile navigation, an Editor Shell Preview for visual evaluation, crash-recovery checkpoint APIs, and the Phase 4F local IPC/semantic CLI foundation. Headless CLI commands can summarize, rename, and explicitly inspect/apply/discard recovery for `.orproj` files. Attached CLI commands connect through an explicit descriptor to the developer/headless `or session serve` host; attached rename requires a separate explicit save. Unix endpoints use owner-only permissions; the Windows runtime directory, descriptor, and named pipe use protected owner-only DACLs, and Windows pipes reject remote clients. The Flutter application does not yet create/open/save real projects or host the live session used by attached CLI clients. There is no recovery UI or autosave. The Editor Shell Preview is not a working editor. There is no usable timeline editor, media ingest, playback, rendering, or export. Project formats and features may evolve before a stable release.
