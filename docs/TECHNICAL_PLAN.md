...
The current `.orproj` schema is v2. The v2 envelope keeps the same format marker and project identity/revision/name, adds an ordered `media` array to project state, and serializes an empty library as `"media": []`. `ProjectDocument` stores a typed UUIDv4 `ProjectId`, persistent `ProjectRevision`, UTF-8 name, and ordered media items. Its fields are not the wire schema: private versioned DTOs and explicit conversion keep internal domain changes from silently changing the file contract. V1 and v2 decoding validate IDs and required fields and reject unknown fields; the version probe rejects unsupported versions before version-specific decoding. The Rust encoder emits deterministic pretty v2 JSON with a trailing newline for the same document; this is not a cross-implementation canonical JSON standard. `ProjectInstanceId` is runtime-only and is never persisted. Phase 4E1 initially wrapped the v1 codec in bounded filesystem load/save; Phase 5B adds the v1-to-v2 migration. Future persistent object IDs should follow the same typed opaque-ID pattern unless evidence justifies another representation. No `TrackId`, `ClipId`, or other object IDs exist yet.

V1 contains only project ID, revision, and name. Decoding v1 constructs an in-memory v2 `ProjectDocument` with an empty media library and preserves those three values. A clean open does not rewrite the source file. The next explicit save encodes v2, and conversion alone does not increment project revision; a later successful `media.add` is a canonical mutation and increments it once. V2 media entries contain a UUIDv4 `MediaId`, `MediaSourceRef::LocalFile` with a validated local `file:` URI, and validated `MediaMetadata`; source bytes remain external. The URI is limited to 8,192 bytes. Media IDs and source URIs must each be unique within a project, and array order is insertion order. V2 metadata is strictly bounded and rejected rather than truncated; exact limits are recorded in [SECURITY_LICENSING.md](SECURITY_LICENSING.md).
...
`ProjectRevision` is a persistent canonical project-state value backed by an unsigned 64-bit integer. Phase 4A implements its initial value, zero, and checked increment; the v1/v2 codecs preserve the stored revision during encode/decode. A changed rename, media add/remove, or transaction increments once; an exact no-op, failed operation, net-no-op transaction, and read-only query leave the revision unchanged. Undo and redo restore content through new canonical mutations, so they increment from the current revision rather than moving it backward. V1-to-v2 schema conversion during explicit save does not increment it. Project ID and revision survive save/reopen; each fresh runtime open receives a new ephemeral `ProjectInstanceId`, which is excluded from the project document.
...
Stable errors include `UNKNOWN_COMMAND`, `UNSUPPORTED_COMMAND_SCHEMA`, `UNKNOWN_QUERY`, `UNSUPPORTED_QUERY_SCHEMA`, `UNSUPPORTED_TRANSACTION_SCHEMA`, `EMPTY_TRANSACTION`, `COMMAND_NOT_ALLOWED_IN_TRANSACTION`, `PROJECT_ID_MISMATCH`, `PROJECT_INSTANCE_MISMATCH`, `REVISION_CONFLICT`, `INVALID_ARGUMENTS`, `REVISION_OVERFLOW`, `MEDIA_ID_ALREADY_EXISTS`, `MEDIA_SOURCE_ALREADY_EXISTS`, `MEDIA_NOT_FOUND`, `NOTHING_TO_UNDO`, `NOTHING_TO_REDO`, `HISTORY_CONFLICT`, and `HISTORY_STORAGE_FAILURE`. Probe/preparation errors remain separate media-probe/media-import errors. The CLI, local IPC client, and Flutter bridge use the shared operation errors. Permissions, dry-run, generalized transaction operations, timeline edits, and agent clients remain future work.

## 5. Query system

The deterministic query catalog contains `project.summary` schema v1 and `media.list` schema v1. `QueryEnvelope` v1 has `query_id`, `schema_version`, typed `project_id`, typed `project_instance_id`, and `arguments`. `project.summary` accepts an empty object and retains its existing wire shape: project ID, runtime instance ID, current revision, and name. `media.list` accepts unsigned `offset` and `limit`, rejects zero or limits above 100, returns a bounded page in project insertion order, and includes items, total count, offset, limit, and `next_offset` when more items exist. An offset beyond the end returns a valid empty page. Both queries read canonical state without mutation or revision changes; listing does not open/probe source files, and offline sources remain listable.

Unknown queries and unsupported query schemas return `UNKNOWN_QUERY` and `UNSUPPORTED_QUERY_SCHEMA`; project/session mismatches use the corresponding shared codes, and invalid arguments return `INVALID_ARGUMENTS`. Timeline, selection, caption, and other query families remain future work. Queries must not start hidden destructive work or return provider credentials. Output fields and schema versions are discoverable for automation clients. IPC protocol v1 and the `project.summary` wire result are unchanged by `media.list`.

## 6. History and transactions

Phase 4D implements the first in-memory transaction and history behavior without event sourcing. `ChangeSet` supports a normalized before/after project-name change plus indexed media-added and media-removed changes. A successful rename or media command creates one compact history entry; media entries store the item and its insertion index, not full `ProjectDocument` snapshots. Undo/redo restore the same media identity, metadata, and order; each undo/redo is itself a new canonical mutation and increments the current revision once. New real edits clear redo history. No-op edits, failed operations/groups, and reads leave project state, revision, and history unchanged. History lives only inside `ProjectSession`: it is not saved in `.orproj` and resets when the document is opened into a new session.

The transaction envelope currently accepts only `project.rename` child calls. Grouped commands stage all intermediate names and normalize them to one net `ChangeSet`; a net no-op does not increment revision or create history. This proves bounded single-operation atomic grouping, not a general transaction framework. The separate project-storage and recovery-checkpoint APIs do not depend on keeping an unbounded event log; persistent history and future schema migrations remain future work.
...
The Flutter bridge uses `flutter_rust_bridge` 2.13.0 with generated typed bindings in `packages/or_app_bridge` and a thin `crates/or_app_bridge` adapter. Its opaque `ProjectHostHandle` owns a `LiveProjectHost`; it does not expose mutable `ProjectDocument` state to Dart. Typed operations cover create/open, summary, rename, undo/redo, save, close, recovery inspection/actions, bounded media-page query, import, remove, descriptor path, and ordered invalidation events. Flutter keeps an immutable active-project read model and refreshes it from Rust after events; mutations carry the read model's expected revision and do not auto-retry conflicts. The desktop `file_selector` plugin chooses project/media paths only; Rust owns canonical file validation, preparation, commands, and persistence. The Media panel shows filename, format, duration, video/audio summary, and source/MediaId on demand, with 50-item pages over the core's 100-item page limit. Import requires system `ffprobe`. If the backend is missing, Flutter shows:
...
Phase 5A implements `JobId` (validated UUIDv4), `JobKind::MediaProbe`, and `JobState` values (`Queued`, `Running`, `Succeeded`, `Failed`, and `Cancelled`). `JobKind::MediaProbe` remains the only concrete kind, and the public media probe still runs synchronously.
...
Phase 5C implements a disposable filesystem `CacheStore` (`crates/or_core/src/cache.rs`). The caller supplies an explicit root and a non-zero `CacheStoreConfig` (`max_entry_bytes`, `max_total_bytes`). It supports two namespaces, `CacheArtifactKind::{Thumbnail, Waveform}`, and stores opaque bounded artifact bytes; no artifact format is chosen. A `CacheKey` is a deterministic SHA-256 digest over `CACHE_SCHEMA_VERSION = 1`, the artifact kind, a `SourceFingerprint`, and a `ParametersFingerprint`, displayed as 64 lowercase hexadecimal characters; `DefaultHasher` is not used. Paths derive only from typed keys (`<root>/<namespace>/<first-two-hex>/<key>.cache`); no caller path segment or source filename enters the path. Entries over `max_entry_bytes` are rejected before any filesystem mutation; a write that would raise the store above `max_total_bytes` (accounting for replacing an existing key rather than double-counting it) returns `CacheError::BudgetExceeded` without deleting existing entries. Writes are atomic via a same-directory `create_new` temp file, flush, file sync, and rename; reads are bounded to `max_entry_bytes + 1`; a missing entry is a normal miss; `remove`, `clear_namespace`, and `clear_all` are explicit. Phase 5C defines the fingerprint contracts but does not acquire production source fingerprints, does not define generation parameters, and implements no automatic eviction, cache database, index, or thumbnail/waveform generator. Cache contents are never canonical project state and are never stored in `.orproj`, the recovery sidecar, or history.
